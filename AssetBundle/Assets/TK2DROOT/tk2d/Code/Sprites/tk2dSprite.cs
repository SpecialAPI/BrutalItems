...
public class tk2dSprite : tk2dBaseSprite
{
	...
	void OnDrawGizmos() {
		if (collectionInst != null && spriteId >= 0 && spriteId < collectionInst.Count) {
			...
			Gizmos.DrawCube(Vector3.Scale(sprite.untrimmedBoundsDataCenter, _scale), Vector3.Scale(sprite.untrimmedBoundsDataExtents, _scale));
	...
	public override void ReshapeBounds(Vector3 dMin, Vector3 dMax) {
		...
		Vector3 oldMin = Vector3.Scale(sprite.untrimmedBoundsDataCenter, _scale) - 0.5f * Vector3.Scale(sprite.untrimmedBoundsDataExtents, oldAbsScale);
		Vector3 oldSize = Vector3.Scale(sprite.untrimmedBoundsDataExtents, oldAbsScale);
		Vector3 newAbsScale = oldSize + dMax - dMin;
		newAbsScale.x /= sprite.untrimmedBoundsDataExtents.x;
		newAbsScale.y /= sprite.untrimmedBoundsDataExtents.y;
		// Clamp the minimum size to avoid having the pivot move when we scale from near-zero
		if (sprite.untrimmedBoundsDataExtents.x * newAbsScale.x < sprite.texelSize.x * minSizeClampTexelScale && newAbsScale.x < oldAbsScale.x) {
			dMin.x = 0;
			newAbsScale.x = oldAbsScale.x;
		}
		if (sprite.untrimmedBoundsDataExtents.y * newAbsScale.y < sprite.texelSize.y * minSizeClampTexelScale && newAbsScale.y < oldAbsScale.y) {
			dMin.y = 0;
