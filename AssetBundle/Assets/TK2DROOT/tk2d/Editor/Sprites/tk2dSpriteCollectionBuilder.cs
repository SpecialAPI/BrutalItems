...
#pragma warning restore CS0618 // Type or member is obsolete
	...
	static void PadTexture(Texture2D tex, int pad, tk2dSpriteCollectionDefinition.Pad padMode)
	{
		...
		if (padMode == tk2dSpriteCollectionDefinition.Pad.Default) {
			padMode = tk2dSpriteCollectionDefinition.Pad.BlackZeroAlpha;
		}
    ...
    public static bool Rebuild(tk2dSpriteCollection gen)
    {
		...
		for (int atlasIndex = 0; atlasIndex < atlasData.Length; ++atlasIndex)
		{
	        ...
	        if (gen.atlasMaterials[atlasIndex] == null)
	        {
	            ...
	            if (gen.premultipliedAlpha)
	                mat = new Material(Shader.Find("tk2d/BR_PremulVertexColor"));
	            else
	                mat = new Material(Shader.Find("tk2d/BR_BlendVertexColor"));
		...
		for (int i = 0; i < coll.spriteDefinitions.Length; ++i)
		{
			...
			coll.spriteDefinitions[i].metadata = gen.textureParams[i].metadata;
			coll.spriteDefinitions[i].metadata.type = gen.textureParams[i].metadata.type;
			gen.textureParams[i].metadata.type = gen.textureParams[i].metadata.type;
...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    {
		coll.SpriteIDsWithAttachPoints.Clear();
		coll.SpriteDefinedAttachPoints.Clear();
        for (int i = 0; i < sourceTextures.Length; ++i)
        {
            ...
            if (mesh)
            {
				coll.spriteDefinitions[i].position0 = meshTransform.TransformPoint(mesh.vertices[0]);
				coll.spriteDefinitions[i].position1 = meshTransform.TransformPoint(mesh.vertices[1]);
				coll.spriteDefinitions[i].position2 = meshTransform.TransformPoint(mesh.vertices[2]);
				coll.spriteDefinitions[i].position3 = meshTransform.TransformPoint(mesh.vertices[3]);
            ...
            else
            {
				...
				coll.spriteDefinitions[i].position0 = positions[0];
				coll.spriteDefinitions[i].position1 = positions[1];
				coll.spriteDefinitions[i].position2 = positions[2];
				coll.spriteDefinitions[i].position3 = positions[3];
			...
			coll.spriteDefinitions[i].boundsDataCenter = (boundsMax + boundsMin) / 2.0f;
			coll.spriteDefinitions[i].boundsDataExtents = (boundsMax - boundsMin);
			...
			if (mesh)
			{
				...
				coll.spriteDefinitions[i].untrimmedBoundsDataCenter = coll.spriteDefinitions[i].boundsDataCenter;
				coll.spriteDefinitions[i].untrimmedBoundsDataExtents = coll.spriteDefinitions[i].boundsDataExtents;
			}
			else
			{
				boundsMin = Vector3.Min(untrimmedPos0, untrimmedPos1);
				boundsMax = Vector3.Max(untrimmedPos0, untrimmedPos1);
				coll.spriteDefinitions[i].untrimmedBoundsDataCenter = (boundsMax + boundsMin) / 2.0f;
				coll.spriteDefinitions[i].untrimmedBoundsDataExtents = (boundsMax - boundsMin);
			}
    ...
    static void UpdateAttachPointData(tk2dSpriteCollection gen, float scale, tk2dSpriteCollectionData target, int spriteId, Vector3 origin) {
		...
		target.SpriteIDsWithAttachPoints.Add(spriteId);
		target.SpriteDefinedAttachPoints.Add(new AttachPointData(def.attachPoints.ToArray()));
    }
	...
	static void UpdateColliderData(tk2dSpriteCollection gen, float scale, tk2dSpriteCollectionData coll, int spriteIndex, Vector3 origin)
	{
		...
		if (colliderType == tk2dSpriteCollectionDefinition.ColliderType.BoxTrimmed)
		{
			...
			def.colliderVertices[0] = def.boundsDataCenter;
			def.colliderVertices[1] = def.boundsDataExtents * 0.5f; // extents is 1/2x size
