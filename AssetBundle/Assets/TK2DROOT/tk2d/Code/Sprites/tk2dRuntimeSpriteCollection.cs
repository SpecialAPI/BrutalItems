...
namespace tk2dRuntime
{
	static class SpriteCollectionGenerator 
	{
			...
			bool[] rotated)
		{
			...
			string shaderName = "tk2d/BR_BlendVertexColor";
		...
		static tk2dSpriteDefinition CreateDefinitionForRegionInTexture(string name, Vector2 textureDimensions, float scale, Rect uvRegion, Rect trimRect, Vector2 anchor, bool rotated)
		{
			...
			if (rotated) {
				def.position0 = new Vector3(-pos1.y + offset.x, pos0.x + offset.y, 0);
				def.position1 = new Vector3(-pos0.y + offset.x, pos0.x + offset.y, 0);
				def.position2 = new Vector3(-pos1.y + offset.x, pos1.x + offset.y, 0);
				def.position3 = new Vector3(-pos0.y + offset.x, pos1.x + offset.y, 0);
				def.uvs = new Vector2[] {
			...
			{
				def.position0 = new Vector3(pos0.x + offset.x, pos0.y + offset.y, 0);
				def.position1 = new Vector3(pos0.x + offset.x, pos0.y + offset.y, 0);
				def.position2 = new Vector3(pos0.x + offset.x, pos1.y + offset.y, 0);
				def.position3 = new Vector3(pos1.x + offset.x, pos1.y + offset.y, 0);
				def.uvs = new Vector2[] {
					new Vector2(v0.x,v0.y),
					new Vector2(v1.x,v0.y),
					new Vector2(v0.x,v1.y),
			...
			def.boundsDataCenter = (boundsMax + boundsMin) / 2.0f;
			def.boundsDataExtents = (boundsMax - boundsMin);

			def.untrimmedBoundsDataCenter = (boundsMax + boundsMin) / 2.0f;
			def.untrimmedBoundsDataExtents = (boundsMax - boundsMin);
